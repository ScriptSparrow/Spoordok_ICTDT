...
public class InMemoryChatHistoryManager implements IChatHistoryManager {
    ...
    public Map<UUID, Queue<HistoricalMessage>> conversationHistories = new HashMap<UUID, Queue<HistoricalMessage>>();
    ...
    public void createHistoryIfNotExists(UUID conversationId, HistoricalMessage systemMessage) {
        if (!conversationHistories.containsKey(conversationId)) {
            Queue<HistoricalMessage> newHistory = new ArrayDeque<>();
    ...
    public List<HistoricalMessage> getHistory(UUID conversationId, int maxMessages) {
        if (!conversationHistories.containsKey(conversationId)) {
            return new LinkedList<HistoricalMessage>();
        }

        Queue<HistoricalMessage> fullHistory = conversationHistories.get(conversationId);
        ...
        List<HistoricalMessage> historyList = new LinkedList<>(fullHistory);
